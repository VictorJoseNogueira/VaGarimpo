...
def run_module(module_name: str) -> int:
    ...
    process = subprocess.run([sys.executable, "-m", module_name], cwd=ROOT_DIR, env=env)
    return process.returncode


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ponto de entrada para executar o scraper e o pipeline de processamento de vagas."
    )
    parser.add_argument(
        "--mode",
